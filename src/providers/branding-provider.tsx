...
function hexToRgbTriplet(hex: string): string {
  ...
  if (!/^[0-9a-fA-F]{6}$/.test(t)) return '37 99 235';
  return `${parseInt(t.slice(0, 2), 16)} ${parseInt(t.slice(2, 4), 16)} ${parseInt(t.slice(4, 6), 16)}`;
}

function hexToDarkRgbTriplet(hex: string): string {
  // Very dark tonal variant (L=7%, S=38%) of the brand hue — used for deep surfaces.
...
function hexToHslTriplet(hex: string): string {
  const t = hex.replace(/^#/, '').trim();
  if (!/^[0-9a-fA-F]{6}$/.test(t)) return '217 91% 60%';
...
export function BrandingProvider({ children }: { children: ReactNode }) {
  ...
  useMemo(() => {
    if (typeof window !== 'undefined') {
      ...
      // Derive deep tonal variants from the brand hue (parity with pos-ui/library-ui)
